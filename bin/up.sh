...
docker run -d \
  ...
  -v /tmp/worker:/tmp/worker \
  192.168.0.13:5000/tf-gpu-builder \
  java -Xmx5g -Djava.util.logging.config.file=/config/prod.logging.properties -jar buildfarm-shard-worker_deploy.jar /config/shard-worker-16.config --public_name=192.168.0.10:8981
EOF
...
docker run -d \
  ...
  -v /tmp/worker:/tmp/worker \
  192.168.0.13:5000/tf-gpu-builder \
  java -Xmx5g -Djava.util.logging.config.file=/config/prod.logging.properties -jar buildfarm-shard-worker_deploy.jar /config/shard-worker-16.config --public_name=192.168.0.11:8981
EOF
